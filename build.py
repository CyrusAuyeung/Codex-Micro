...
subprocess.run(base+['/target:exe','/r:System.Windows.Forms.dll','/out:'+str(root/'MicroInput.Windows.exe'),str(root/'native/MicroInput.cs')],check=True)
...
subprocess.run([str(root/'MicroInput.Windows.exe'),'--self-test'],check=True)
subprocess.run([str(root/'runtime/node.exe'),'--check',str(root/'server.mjs')],check=True)
print('Built Micro Windows launcher, HID, network and input helpers (Windows x64).')
