...
assert.ok(server.includes('version:APP_VERSION,simulation'), 'Server must use central version');
const launcher = await read('native/Launcher.cs');
assert.ok(launcher.includes('Assembly.GetExecutingAssembly().GetName().Version'), 'Launcher must use build version');
